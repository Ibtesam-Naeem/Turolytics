...
class TuroService {
  ...
  // Scraping
  async scrape(scraperType: 'all' | 'vehicles' | 'trips' | 'reviews' | 'earnings' | 'transactions' | 'receipts', email?: string, password?: string): Promise<{ task_id: string; account_id: number; scraper_type: string }> {
