...
def generate_trips_data(vehicle_plates: List[str]) -> Dict[str, Any]:
    ...
    # Generate trips that are checking out today (end_date is today)
    for i in range(3):
        # Start date is a few days ago, end date is today
        days_ago = random.randint(1, 5)
        start_date = now - timedelta(days=days_ago)
        end_date = now  # End date is today
        ...
        booked_trips.append({
            ...
            "kilometers": {
                "kilometers_included": random.randint(200, 1000),
                "kilometers_driven": random.randint(100, 400),  # Some kms already driven
                "overage_rate": round(random.uniform(0.30, 0.50), 2),
            },
            "earnings": {
                "total_earnings": round(random.uniform(50, 500), 2),
